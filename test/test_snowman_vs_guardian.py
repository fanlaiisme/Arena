...
def main():
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            print(f"  [{pair_count:2d}/{total_pairs}] {_pad(c1.name, 12)} vs {_pad(c2.name, 12)}  "
                  f"{wins[c1.name]:3d} : {wins[c2.name]:3d}  "
    ...
    # 矩阵表头
    header = " " * name_w + "".join(_pad(c.name, col_w) for c in chars) + "  总胜"
    lines.append(header)
    ...
    for i, ci in enumerate(chars):
        ...
        lines.append(_pad(ci.name, name_w) + "".join(cells) + f"  {total_wins}")
    lines.append("")
    ...
    for i, c in enumerate(chars):
        ...
        rankings.append((c.name, tw, (n - 1) * MATCHES_PER_PAIR))
    rankings.sort(key=lambda x: x[1], reverse=True)
    for rank, (name, wins, total) in enumerate(rankings, 1):
        lines.append(f"  {rank}. {_pad(name, 12)}  {wins:4d}/{total}  ({wins/total*100:5.1f}%)")
    ...
    json_data = {
        ...
        "matrix": {f"{chars[i].id}_vs_{chars[j].id}": {"p1_wins": matrix[i][j], "p2_wins": matrix[j][i]}
                   for i in range(n) for j in range(i + 1, n)},
        "rankings": [{"name": name, "wins": wins, "total": total} for name, wins, total in rankings],
    }
